...
export default function Checkout() {
  ...
  const [address, setAddress] = useState({ line1: "", city: "Brooklyn", notes: "" });
  ...
  async function placeOrder() {
    ...
    if (!address.line1) { toast.error("Address required"); return; }
    setLoading(true);
    const { data: order, error } = await supabase.from("orders").insert({
      customer_id: user.id, restaurant_id: restaurantId!,
      subtotal, delivery_fee: fee, total, payment_method: payment,
      address_line: address.line1, city: address.city, notes: address.notes || null,
    ...
    // Pre-create delivery row (unassigned) so partners can pick it up
          ...
          <div className="space-y-6">
            <div className="card-flat p-6">
              ...
              <div className="space-y-3">
                <div><Label>Street address</Label><Input value={address.line1} onChange={(e) => setAddress({ ...address, line1: e.target.value })} placeholder="123 Bedford Ave, Apt 4B" /></div>
                <div><Label>City</Label><Input value={address.city} onChange={(e) => setAddress({ ...address, city: e.target.value })} /></div>
                <div><Label>Notes (optional)</Label><Textarea value={address.notes} onChange={(e) => setAddress({ ...address, notes: e.target.value })} placeholder="Buzzer, gate code, allergies..." rows={2} /></div>
              </div>
            </div>
